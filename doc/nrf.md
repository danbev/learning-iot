...
BLE_LBS_DEF(m_lbs);
```
There is a target named 'pre' in the nrf/ble directory which will output the
code generated by the pre-processor:
```c
...
integer or string literal operand. 
...
Looking at the above assembly there is first a test using `tst` using the linnk
register. Next we have the `it` instruction, which is like if-then and then
